...
%     y = Tau_Total.Normalized(contains(Tau_Total.Treatment,'DMSO') & contains(Tau_Total.Treatment,'1000 ng/mL doxycyclin'))
    ...
    for drug = 1:size(uni_Drugs,1)
        ...
        for experiment = 1:size(Unique_Experiments,1)
            ...
            for kk = 1:size(x,1)
                ax = gca; ax.ColorOrderIndex = coloridx;
...
%             OldStr = lh.String
%             lh.String = [OldStr {'test'}]
            coloridx=coloridx+1; 
        end
        grid on;title('Normalized Cell Cycle for CycD RPE1');lh = legend(Unique_Experiments,'Interpreter', 'none');hold off;
    end
    
    figure(); hold on;
    for kk = 1:4:size(y,1)
        kk
        x_set = categorical(x(kk:kk+3))
        y_set = (y(kk:kk+3))
        scatter(x_set,y_set)
    end
    grid on
    title('Normalized Cell Cycle for CycD RPE1')
    fgh = legend(Unique_Experiments,'Interpreter', 'none')
